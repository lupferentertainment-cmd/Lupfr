...
import { useInView, useReducedMotion, motion, AnimatePresence, useScroll, useTransform } from "framer-motion"
...
function MarqueeTrack({
  ...
  const content = (
    <>
      {reviews.map((r) => (
        <ReviewItemBlock key={r.id} review={r} layer={layer} />
      ))}
    </>
  )
  ...
  return (
    <div
      className={`reviews-marquee-outer reviews-marquee-strip--${layer} overflow-hidden py-1 sm:py-2`}
    >
      <div
        className={`reviews-marquee-inner reviews-marquee-inner--${speed} flex gap-3 sm:gap-4 w-max ${
          direction === "left"
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 max-w-4xl mx-auto">
      {stats.map((stat) => (
        <div key={stat.label} className="text-center">
          <p className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight tabular-nums">
            {stat.type === "decimal" ? (
...
export function Reviews() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-8 sm:py-10 bg-muted/30 border-y border-border/50"
      aria-label="Reviews"
    >
        ...
        <div className="min-h-[140px] sm:min-h-[160px] md:min-h-[180px] flex items-center justify-center">
          <AnimatePresence mode="wait" initial={false}>
            <motion.div
              ...
              className="w-full"
            >
              <StatsCarouselSlide stats={currentStatsSlide} isActive={isStatsInView} />
            </motion.div>
          </AnimatePresence>
        </div>
        {/* Carousel indicators */}
      ...
      <div className="flex flex-col gap-0 [perspective:1200px] [transform-style:preserve-3d]">
        <MarqueeTrack
