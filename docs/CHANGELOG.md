...
- Added canonical `bun run test:suite` plus focused `bun run test:smoke` and `bun run test:gallery` scripts. `bun run ci`, pre-commit, and Vercel `ci:vercel` now share the same full suite, including coverage, build/typecheck, route/external-link QA, and Playwright browser console/runtime crawl.
- Added `tests/unit/test-suite-gate.test.ts` to lock the single-gate contract so `bun run test:suite` keeps running all Vitest tests, gallery journey coverage, dynamic route/external-link QA, and browser runtime crawl.
- Updated the installed pre-commit hook source so commits run the public raster gate and then `bun run test:suite` directly.
