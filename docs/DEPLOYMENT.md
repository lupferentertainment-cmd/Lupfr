...
**SEA // SIDE microsite domain.** `seaside.lupfr.com` is served by this same project via the host rewrite in `proxy.ts` (seaside hosts rewrite to `app/seaside`). Attach the domain once in the dashboard: **lupfr project → Settings → Domains → add `seaside.lupfr.com`** (lupfr.com already uses Vercel nameservers, so it verifies instantly; no DNS records needed). Until attached, `seaside.lupfr.com` answers `DEPLOYMENT_NOT_FOUND`. Locally: `http://seaside.localhost:3000`.

**SEA // SIDE staging domain (`dev.seaside.lupfr.com`).** For a staging URL of the microsite, add `dev.seaside.lupfr.com` in **Settings → Domains** and **assign it to the `dev` branch** (Domains → the domain → **Edit → Git Branch: `dev`**), so it always serves the current `dev` Preview build. `proxy.ts` already lists `dev.seaside.lupfr.com` as a seaside host, so it rewrites to `/seaside` like the production host. Mirrors the staging→prod model: review on `dev.seaside.lupfr.com`, promote to `seaside.lupfr.com` via `bun run promote:prod`.
