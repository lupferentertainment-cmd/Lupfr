...
**SEA // SIDE microsite — decommissioned (owner decision 2026-07-08).** SEA // SIDE now lives at its own domain and Vercel project, **`seaside.la`** (project `seaside-lupfr`, source repo `~/dev/lupfr-seaside`). The old microsite hosted in this repo is retired: `proxy.ts` **308-redirects** the legacy prod host `seaside.lupfr.com` (and `seaside.localhost` for local dev) — and the primary-host `/seaside` page route — to `https://seaside.la/`, so old links and search ranking transfer to the new site. **Keep `seaside.lupfr.com` attached to the lupfr project** so the redirect fires; detaching it would return `DEPLOYMENT_NOT_FOUND` instead of a clean 308. **The old staging host `dev.seaside.lupfr.com` is retired (owner decision 2026-07-08)** — staging now lives at **`dev.seaside.la`** on the seaside project; it was dropped from `SEASIDE_HOSTS` in `proxy.ts`. **Detach `dev.seaside.lupfr.com` from the lupfr Vercel project** (Settings → Domains) so it no longer serves the main site. The `app/seaside` page + `components/seaside/*` code and `public/seaside/*` assets are retained (still unit-tested) but no longer publicly routed; full code/asset removal is a follow-up. `/seaside` is excluded from `app/sitemap.ts`. **History (pre-2026-07-08):** `seaside.lupfr.com` (prod) and `dev.seaside.lupfr.com` (pinned to the `dev` branch) served the in-repo microsite via a host rewrite to `app/seaside`, promoted staging→prod via `bun run promote:prod`; Vercel Authentication gated the `dev.*` hosts while production stayed public.
...
**CI (GitHub Actions).** On every push and pull request, `.github/workflows/ci.yml` installs Chromium via `scripts/install-playwright-chromium.sh` and runs `bun run test` — the same full gate Vercel runs in `buildCommand`.
