...
**CI (GitHub Actions).** On every push and pull request, `.github/workflows/ci.yml` runs `bun run ci:vercel` (same as the Vercel project **Build Command**): public raster check, lint, Vitest with coverage thresholds, **`bunx --bun next build`**, then `verify:routes` (`scripts/verify-routes.sh`). The route script starts **`bun run start`** (Next production server via **`bunx --bun next start`**) and asserts `/`, every `/events/[slug]` from generated data, and a synthetic missing path (expects HTTP 404), plus internal-doc URLs stay 404. Failing the GitHub job does not block Vercel by itself; the Vercel build still runs the identical script, so both surfaces catch the same issues.
