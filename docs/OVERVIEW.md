...
**Content model.** Site content is data-driven: YAML in `data/` (events, gallery, artists, services, partners) is compiled at build time by `scripts/generate-data.js` into JSON under `lib/data/generated/`. Raster images in `public/` (except `favicon/` and `logos/`) are **WebP**; `bun run test` enforces that before every ship. No CMS; content changes = edit YAML + run the single verification gate. A password-gated **operator shell** at `admin.lupfr.com` / `/admin` offers read-only ops (live Vercel Analytics charts, contacts/telemetry via optional Supabase, content counts) — it does not edit YAML or media.
