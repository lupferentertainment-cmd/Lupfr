...
**High-level.** Single Next.js app (App Router). Static/semi-static pages driven by generated JSON; three API routes (`contact`, `newsletter`, `phone-list`). No database; no auth. Build step compiles YAML → JSON before `next build`.
...
- **API:** `app/api/contact/route.ts` (POST, form → Resend), `app/api/newsletter/route.ts` (POST, email → Resend; sends internal notification + welcome to subscriber), `app/api/phone-list/route.ts` (POST, name + phone → Google Sheets webhook).
