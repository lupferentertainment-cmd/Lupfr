...
- `bun run verify:routes` – production route smoke checks; also asserts **404** for internal-doc URLs (`/docs`, `/docs/overview`, `/README.md`, `/api.md`) so they never become accidental public routes (see `proxy.ts`). Unknown paths must return **404** (not streamed 200): the app enables **`experimental.globalNotFound`** and `app/global-not-found.tsx` so crawlers and this script see a real status code.
