...
- `bun run lint` – ESLint.
- `bun run typecheck` – strict TypeScript validation via `bunx tsc --noEmit`.
- `bun run build` – `generate-data` + strict `typecheck` + `scripts/clean-next-dist.mjs` + `bunx --bun next build --webpack`; `next.config.mjs` keeps Next’s internal `ignoreBuildErrors` enabled because the explicit typecheck gate owns TypeScript failure reporting. The cleaner removes `NEXT_DIST_DIR` when set and refuses to remove `.next` while an active `next dev` process is using this repo.
- Next static generation is constrained with `experimental.cpus: 1` so build worker processes do not race `.next` cleanup before `verify:client-bundle` and `verify:routes` inspect the production output.
...
- `bun run verify:routes` – production route smoke checks; asserts public crawl/discovery endpoints respond (`/robots.txt`, `/sitemap.xml`, `/site.webmanifest`, `/llms.txt`, `/opengraph-image`, `/twitter-image`, favicon root/folder fallbacks, and the legacy `/layout.css` no-op stylesheet), and still enforces **404** for internal-doc URLs (`/docs`, `/docs/overview`, `/README.md`, `/api.md`) so they never become accidental public routes (see `proxy.ts`). Unknown paths must return **404** through the standard App Router `app/not-found.tsx` path; the unstable experimental global-not-found build path stays disabled.
- **Dynamic link QA (internal + external).** `verify:routes` crawls every internal route reachable from `/` (each must return **200**) via `scripts/extract-internal-routes.mjs`, and additionally health-checks **every external `<a href>`** discovered while crawling (Instagram, TikTok, YouTube, Partiful, the Google Calendar “book a call” CTA, etc.) via `scripts/extract-external-links.mjs`. External links are fetched with a browser User-Agent and a tolerant policy: a host that answers — even with a bot-block `401/403/405/429` — counts as **alive**; only a **DNS/connection failure**, **404/410**, or **5xx** fails the run, and a timeout is a non-fatal `WARN`. This runs in `verify`, `ci`, and on **Vercel** (`ci:vercel`). Set `VERIFY_ROUTES_SKIP_EXTERNAL=1` to skip just the outbound checks for offline local runs. The static `LINKS` shape contract stays in `tests/unit/links.test.ts`, and the external-link extractor is unit-tested in `tests/unit/extract-external-links.test.ts`.
- **Same clone as `next dev`:** `bun run ci` and `bun run verify` use an isolated `NEXT_DIST_DIR` under `.next-ci/` so they do not delete `.next/dev`, then fail fast before the production build if `next dev` is still active for this repo. Stop dev before full verification because Next can hang when dev and build run together even with separate output dirs. If **`4310`** is taken, use e.g. **`VERIFY_ROUTES_PORT=43990 bun run verify:routes`**.
- `bun run verify:console` – **headless-browser error gate.** Starts `next start`, crawls every internal route from `/` in **Chromium** (Playwright), and **fails on any `console.error`, uncaught page/runtime error (incl. React hydration #418/#423), or same-origin HTTP `>= 400`**. This catches Next.js client/hydration errors that the `curl`-based `verify:routes` cannot observe. Runs in `bun run ci` after `verify:routes`; the Vercel build (`ci:vercel`) sets `LUPFR_SKIP_BROWSER_CHECK=1` to skip it (no browser binary in the build container). Port via `VERIFY_CONSOLE_PORT`.
- `bun run verify` – lint + build (including typecheck) + client-bundle scan + route smoke.
- `bun run ci` – lint + coverage + isolated build (including typecheck) + client-bundle scan + route smoke through `scripts/ci.sh ci`.
...
**Home performance guardrail.** `tests/unit/home-performance.test.ts` asserts that the home page keeps lower sections behind intersection-observed hash placeholders, uses a larger mobile observer margin so Gallery mounts before the user reaches it, keeps mobile placeholders compact, keeps immediately visible Reviews as a normal import to avoid unused dynamic chunk preload warnings, and lazy-loads Spotify/SoundCloud iframes without extra opt-in buttons. This protects mobile transfer, avoids confusing blank scroll gaps, and keeps the artist card UX consistent across light and dark modes.
